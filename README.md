...
|---------|---------|-------------|
| `deja seed` | Build the demo BEFORE state: typed nodes + explicit edges, **zero** inferred edges | — |
| `deja start` | Cold open — recall who you are from graph state, including your last session | — |
...
| `deja memify` | Re-organize the graph: infer `SAME_FAMILY_AS` links + re-weight skills (idempotent) | — |
...
| `deja doctor` | Print resolved env / data dir / LLM config / key status | — |
| `deja capture` | Write BEFORE/AFTER PNGs of the graph (Scene 3 fallback; needs the `[capture]` extra) | `--host`, `--port` |
| `deja version` | Print the installed version | — |
